...
use another method that is once again a variant of an exhaustive search. Recall
...
coordinate by coordinate. % Can we really say that?
Let 
\[
  xy = (\pi_1(x, y), \dots, \pi_k(x, y)),
\]
where, for all $1\leq i\leq k$, $\pi_i$ is the bilinear form corresponding to
the $i$-th coordinate of the product in $\mathbb{F}_{p^k}$.
% les pi sont des matrices
% explication du coordonnée par coordonnée
