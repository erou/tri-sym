...
As mentionned in Section~\ref{sec:symtrisym}, we are not able to find a
finite field extension $\A=\mathbb{F}_{q^k}$ where the bilinear complexity and
the trisymmetric bilinear complexity are different. Moreover, if the
characteristic of $\K=\mathbb{F}_q$ is not $2$, we can prove that we have
\[
  \mu_q(2) = \tri{q}{2} = 3.
\]
...
$q$ not divisible by $2$. In order to obtain such a formula, it is useful to
change our point of
view of the problem. Assume we want to compute a trisymmetric decomposition of
the product $\Phi$ in $\A$, a commutative algebra of degree $k$. After the choice of a basis of
$\A$ and a basis of the space $\B$ of the bilinear forms on
$\A$, we can represent
\[
  \Phi=(\pi_1, \dots, \pi_k)
\]
...
\[
  AX = B
\]
with the smallest possible number of nonzero entries in $X$. If
$\A=\mathbb{F}_{q^2}$ is not of characteristic $2$, we
...
\[
  ...
  +(x_0y_1+x_1y_0)\alpha.
\]
This formula is the same for all $q$ not divisible by $2$, because we can represent
