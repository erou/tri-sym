...
langage~\cite{Julia} and a package searching for tri-symmetric decompositions is
available online\footnote{\url{https://github.com/erou/TriSym.jl}}, along with
the source code. We were able to algorithmically find tri-symmetric decompositions of the
...
extension where the bilinear complexity and the tri-symmetric bilinear
complexity are different. Moreover, in the case $q\neq2, k=2$, we can prove that we have
\[
  \mu_q(2) = \tri{q}{2} = 3.
\]
...
$\tri{q}{2}$, we find a tri-symmetric formula of length $3$ that is true for all
$q\neq3$. In order to obtain such a formula, it is useful to change our point of
view of the problem. Assume we want to compute a tri-symmetric decomposition of
...
finding a tri-symmetric decompositions of the product in
...
\[
  ...
  +(x_0y_1+x_1y_0)\alpha.
\]
We can then construct the matrix $B$ and $A$, and solve $AX=B$ over
$\mathbb{Q}(\zeta)$, where $\zeta$ is viewed as an indeterminate.
We can finally check that
\[
  B = (1-\zeta^{-1})4^{-1}\bff(1) +
  (8\zeta)^{-1}\bff(1+\alpha)+(8\zeta)^{-1}\bff(1-\alpha),
\]
so that the tri-symmetric bilinear complexity for
